...
class MetricCardsView(ctk.CTkFrame):
    def __init__(self, parent):
        ...
        self.apps_card, self.apps_sub = self.make_card(
            tag="TOTAL APPLICATIONS", title="Pipeline Volume", val="0", sub="All-time targets logged",
            col=0, accent_color="#00f0ff"
        )
        self.interview_card, self.interview_sub = self.make_card(
            tag="IN PROGRESS", title="Active Interviews", val="0", sub="Rounds underway",
            col=1, accent_color="#f59e0b"
        )
        self.offers_card, self.offers_sub = self.make_card(
            tag="OFFERS SECURED", title="Offer Pipeline", val="0", sub="Conversion targets",
            col=2, accent_color="#00f5a0"
        )
        self.conversion_card, self.conversion_sub = self.make_card(
            tag="SUCCESS RATIO", title="Yield Rate", val="0.0%", sub="Offers ÷ Submissions",
            col=3, accent_color="#a855f7"
        )

    def make_card(self, tag, title, val, sub, col, accent_color):
        frame = ctk.CTkFrame(
